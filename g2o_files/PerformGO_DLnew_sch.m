function [ Graph ] = PerformGO_DLnew_sch( Graph )
...
[ NodeTable ] = classifyNodeType( NodeTypeNamesArray, Graph.Schur );

%%%%%%%%%%%%%%% gnerate upstateindex for normal Nodes and schur Nodes
for i=1:Number_NodeTypes 
    ...
    if isfield(Graph.Schur, NodeTypeNamesArray{i})
    else
...
for i=1:Number_NodeTypes 
    ...
    if isfield(Graph.Schur, NodeTypeNamesArray{i})
    Graph.Nodes.( NodeTypeNamesArray{i}  ).updateIndex = num_update+ [1:1: dimension* num_node_thisKind ]';
    NodeTable.schur.( NodeTypeNamesArray{i}  ).updateIndex   = num_update_schur+ [1:1: dimension* num_node_thisKind ]';
    num_update=num_update+ dimension* num_node_thisKind;
    num_update_schur = num_update_schur + dimension* num_node_thisKind;
    end
end
%%%%%%%%%%%%%%%%%%%  gnerate upstateindex for normal Nodes and schur Nodes
...
Whole_Jacobian=[];
Whole_Jacobian_schur=[];
Whole_Jacobian_normal=[];
for i=1:Number_NodeTypes 
    ...
    Total_Jacobian.( NodeTypeNamesArray{i} )=sparse( Graph.Nodes.(ThisTypeName).Jacobian.RowVector, Graph.Nodes.(ThisTypeName).Jacobian.ColVector, Graph.Nodes.(ThisTypeName).Jacobian.ValueVector, num_rows, num_cols   );
        ...
        if isfield(Graph.Schur, NodeTypeNamesArray{i})
             Whole_Jacobian_schur=[Whole_Jacobian_schur  Total_Jacobian.( NodeTypeNamesArray{i} )];
        else
             Whole_Jacobian_normal=[Whole_Jacobian_normal  Total_Jacobian.( NodeTypeNamesArray{i} )];
        end
end

Whole_Jacobian= [ Whole_Jacobian_normal  Whole_Jacobian_schur];

db1 = -Whole_Jacobian_normal'*Total_ErrorVector;
db2 = -Whole_Jacobian_schur'*Total_ErrorVector;

Hnn = Whole_Jacobian_normal'*Whole_Jacobian_normal;
Hns = Whole_Jacobian_normal'*Whole_Jacobian_schur;
Hss = Whole_Jacobian_schur'*Whole_Jacobian_schur;


invHss = calculate_inverse_new( Hss,  NodeTable);

AA = Hns*invHss;
dx1 = (Hnn - AA*Hns')\(db1 -  AA*db2);
dx2 = invHss*(db2 - Hns'*dx1  );



g = -[db1;db2];%  Whole_Jacobian'*Total_ErrorVector;
HH = [ Hnn  Hns; Hns' Hss ];
...
isIncreaseObjValue = 1;

while( isIncreaseObjValue ==1  )
...
for i=1:Number_NodeTypes 
 ...
 if p>0 
    isIncreaseObjValue = 0;  % very important 
