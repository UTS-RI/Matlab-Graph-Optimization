function [ value, inf ] = rk4_manifold_mod3( M, N , accbias_es, gyrobias_es)
...
for j=1:m
  ...
  k4 = h*derivative_sub(j+1,M, specialadd( yj , k3 ), accbias_es, gyrobias_es );  
...
wt=M.gyro(1:3,j);
at=M.acc(1:3,j);
elseif norm(j -floor(j))<= 0.5
    j = floor(j);
    wt = 0.5*M.gyro(1:3,j)+ 0.5*M.gyro(1:3,j+1);  
    at = 0.5*M.acc(1:3,j)+ 0.5*M.acc(1:3,j+1);
else
    j = floor(j);
    wt = 0.5*M.gyro(1:3,j)+ 0.5*M.gyro(1:3,j+1);  
    at = 0.5*M.acc(1:3,j) + 0.5*M.acc(1:3,j+1);
