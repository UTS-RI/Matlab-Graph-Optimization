function [ value, inf ] = rk4_manifold_mod2( M, N , accbias_es, gyrobias_es)
...
for j=1:m
  ...
  k2 = h*derivative_sub(j+1,M, specialadd( yj , k1/2 ), accbias_es, gyrobias_es );
  k3 = h*derivative_sub(j+1,M, specialadd( yj , k2/2 ), accbias_es, gyrobias_es );
  k4 = h*derivative_sub(j+2,M, specialadd( yj , k3 ), accbias_es, gyrobias_es );  
%  Y{j+1}=specialadd( yj,   (k1 + 2*k2 + 2*k3 + k4)/6   );
  Y{j+1}=specialadd( yj,   k1   );
  
  K1 = h*derivative_sub2(j,M,N,Pj, accbias_es, gyrobias_es);
  K2 = h*derivative_sub2(j+1,M,N, Pj+ K1/2 , accbias_es, gyrobias_es );
  K3 = h*derivative_sub2(j+1,M,N, Pj + K2/2 , accbias_es, gyrobias_es);
  K4 = h*derivative_sub2(j+2,M,N, Pj + K3 ,accbias_es, gyrobias_es );
