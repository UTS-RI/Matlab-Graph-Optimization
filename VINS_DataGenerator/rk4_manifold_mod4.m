function [ value, inf ] = rk4_manifold_mod4( M, N , accbias_es, gyrobias_es)
...
for j=1:m
  ...
  k2 = h*derivative_sub(j+0.5,M, specialadd( yj , k1/2 ), accbias_es, gyrobias_es );
  k3 = h*derivative_sub(j+0.5,M, specialadd( yj , k2/2 ), accbias_es, gyrobias_es );
  k4 = h*derivative_sub(j+0.9,M, specialadd( yj , k3 ), accbias_es, gyrobias_es );  
  Y{j+1}=specialadd( yj,   (k1 + 2*k2 + 2*k3 + k4)/6   );
%  Y{j+1}=specialadd( yj,   k1   );
...
if  norm(j -floor(j))< 0.0001
wt=M.gyro(1:3,j);
at=M.acc(1:3,j);
elseif norm(j -floor(j))<= 0.6
    j = floor(j);
    gLL= M.gyro(1:3,j-1);
    gL = M.gyro(1:3,j);
    accLL= M.acc(1:3,j-1);
    ...
    wt = 0.5*(gL-gLL)+ gL;    %0.5*M.gyro(1:3,j)+ 0.5*M.gyro(1:3,j+1);  
    at = 0.5*(accL-accLL)+accL;      %0.5*M.acc(1:3,j)+ 0.5*M.acc(1:3,j+1);
else
    j = floor(j);
    gLL= M.gyro(1:3,j-1);
    gL = M.gyro(1:3,j);
    accLL= M.acc(1:3,j-1);
    accL = M.acc(1:3,j);
    wt = 1*(gL-gLL)+ gL;    %0.5*M.gyro(1:3,j)+ 0.5*M.gyro(1:3,j+1);  
    at = 1*(accL-accLL)+accL;      %0.5*M.acc(1:3,j)+ 0.5*M.acc(1:3,j+1);
end
