...
% 
Measure_translation.inf=eye(6)*1e10;
...
if   1
[ Graph ]= AddComplexEdge(Graph, 'Vision_Factor', NodeArray,  Measurement);
if kk == 0
Graph.Nodes.Landmark3.Values.(landmark_id )= Landmarks.(landmark_id)+0.5*randn(3,1);
end

pose = RobotState{pose_index+1}.pose;
R = pose(1:3,1:3)*expm(skew( randn(3,1)*0));
t = pose(1:3,4)+randn(3,1)*0;
